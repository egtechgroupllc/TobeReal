...
import {IconACB, IconCheck, IconClock, IconLock} from '../assets/icon/Icon';

var hd = new Holidays('VN');

export default memo(function CalendarPriceManage({
  minDate = new Date(),
  startDate,
  endDate,
  onDateChange,
...
const TextPrice = ({
  ...
  if (!isDisable || isDisableToday) return null;
  return isLoading
...
const TextStatus = ({
  ...
  return isLoading
    ? isDisable && <CustomText>...</CustomText>
    : dataDateDetail?.status && isDisable && (
        <View style={{marginTop: scale(5)}}>
          {dataDateDetail?.status === 'ACTIVE' ? (
            <IconCheck></IconCheck>
          ) : (
            <IconLock></IconLock>
          )}
