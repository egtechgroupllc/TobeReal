...
export const formatDate = (
  date = new Date(),
  {dateStyle, addDays: daysToAdd, monthsToAdd, yearsToAdd, addMinutes} = {},
) => {
  ...
  // Thêm số phút vào giờ hiện tại
  if (addMinutes) {
    formattedDateTime.setMinutes(formattedDateTime.getMinutes() + addMinutes);
  }
...
export const formatTime = (
  date = new Date(),
  {isHour24, addMinutes: minutesToAdd} = {},
) => {
  let formattedDateTime = new Date(date);

  // Thêm số phút nếu có chỉ định
  if (minutesToAdd) {
    formattedDateTime = addMinutes(formattedDateTime, minutesToAdd);
  }

  const timeFormat = isHour24 ? 'HH:mm' : 'h:mm a'; // Sửa lại logic kiểm tra 24h
