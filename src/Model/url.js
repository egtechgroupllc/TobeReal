// export const baseUrl = 'https://shome.largeswap.com';
// export const baseUrl = 'https://a59a-115-79-58-50.ngrok-free.app';
// export const baseUrl = 'https://api.saveloka.com';
export const baseUrl = 'https://api.pionehouse.com';
// export const baseUrl = 'https://api.pionehouse.tocuna.com';
export const KEY_MAP = 'AIzaSyA0p4aVomqYcT6mgNv1h169myUhT7mNE28';
export const languageUrlServer = `${baseUrl}/language/server`;
export const languageUrlApp = `${baseUrl}/language/app`;
