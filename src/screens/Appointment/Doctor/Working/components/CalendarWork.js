...
import {getDateWorking} from '~/api/doctor';

var hd = new Holidays('VN');

export default memo(function CalendarWork({
  minDate = new Date(),
  startDate,
  endDate,
  onDateChange,
  ...
  const markedDates = useMemo(() => {
    if (dateSelect.date_end) {
      ...
      return datesInRange.reduce((acc, date) => {
        ...
        acc[dateTime] = {
          color: bgRangeSelect || COLORS.BlueBold,
          textColor: 'white',
        };
        return acc;
      }, {});
...
const BoxText = ({
  ...
  const isSunday = date.accessibilityLabel.includes('Sunday'); // Check if it's Sunday
  ...
  return (
    <View
      style={{
        ...
        backgroundColor:
          (isDateStart || isDateEnd) && (bgSelect || COLORS.cyan),
      ...
      <CText
        ...
        style={{
          ...
          color:
            ...
            (isSunday && date.state !== 'disabled' // Set Sunday text color to red
              ? COLORS.error
