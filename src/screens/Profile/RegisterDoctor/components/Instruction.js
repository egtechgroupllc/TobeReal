...
export default function Instruction({errors, watch, onGoBack, data}) {
  const {t} = useLanguage();
  const [isView, setView] = useState(false);
  const [dataGoBack, setDataGoBack] = useState('');
  const {navigate} = useNavigation();

  const arrKeywords = useRef(['instructions']).current;
  const viewGeneral = () => {
    setView(prev => !prev);
  };
  useEffect(() => {
  ...
  return (
    <View>
      <ButtonTabValidate
        title={t('examination_instructions')}
        onPress={viewGeneral}
        errors={errors}
        watch={watch}
          ...
          <View style={{height: '100%', overflow: 'hidden'}}>
            {data ? (
              ...
              <CText
                ...
                numberOfLines={5}>
                {t('enter_examination_instructions')}
              </CText>
            )}
          </View>
        </TouchableOpacity>
